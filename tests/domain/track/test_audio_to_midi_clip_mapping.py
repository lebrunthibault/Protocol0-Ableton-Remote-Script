...
def test_audio_to_midi_clip_mapping():
    ...
    assert mapping.hash_matches_file_path(1, "path1")
    assert not mapping.hash_matches_file_path(2, "path1")

    assert mapping.file_path_matches_file_path("path1", "path1_bis")
    assert not mapping.file_path_matches_file_path("path1", "path2")

    mapping.register_file_path("path3", ClipInfoTest(3))
    assert not mapping.file_path_matches_file_path("path1", "path3")
    mapping.register_midi_hash_equivalence(1, 3)
    assert mapping.file_path_matches_file_path("path1", "path3")
    assert mapping.file_path_matches_file_path("path1_bis", "path3")
