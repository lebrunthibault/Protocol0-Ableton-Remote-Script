...
def get_code_filenames():
    for current_path, folders, files in os.walk(ROOT_DIR):
        if any(folder_name in current_path for folder_name in
               [".git", "tests", "pytest", "jupyter", ".ipynb_checkpoints", "scripts", "sequence"]):
            continue
        for file in files:
            if file.endswith(".pyc") or file in ("Protocol0.py", "__init__.py"):
                continue
...
def test_sequence_pattern():
    ...
    for filename in get_code_filenames():
        with open(filename, "r") as f:
            ...
            assert instantiated_sequences <= returned_sequences, "invalid sequence code in %s" % filename.replace(ROOT_DIR, "")
...
def test_init_has_arguments():
    ...
    for filename in get_code_filenames():
        with open(filename, "r") as f:
            ...
            assert file_content.count("def __init__") <= file_content.count("super("), "super not called in %s" % base_filename
