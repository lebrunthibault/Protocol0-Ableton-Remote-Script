...
class AutomationTrack(SimpleTrack):
    ...
    def _added_track_init(self):
        ...
        seq.add(lambda: setattr(self, "output_routing_channel", find_last(lambda r: "lfotool" in r.display_name.lower(), self.available_output_routing_channels)))
    ...
    def _create_base_clips(self):
        ...
        for i, (clip_name, velocities) in enumerate(velocity_patterns.items()):
            self.create_clip(slot_number=i, name=clip_name, bar_count=1, notes_callback=partial(self._fill_equal_notes, velocities=velocities))
    ...
    def _fill_equal_notes(self, clip, velocities):
        ...
        return [Note(pitch=vel, velocity=vel, start=i * duration, duration=duration, clip=clip) for i, vel in enumerate(velocities)]
