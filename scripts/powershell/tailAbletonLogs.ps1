...
function Get-LogColor {
    ...
    process {
        if ($LogEntry.Contains("P0 -")) {
            if ($LogEntry.Contains("P0 - info")) {Return "Green"}
            elseif ($LogEntry.Contains("P0 - error") -or $LogEntry.Contains("a_protocol_0")) {Return "Red"}
            elseif ($LogEntry.Contains("P0 - debug")) {Return "Yellow"}
            elseif ($LogEntry.Contains("send_keys")) {Return "Blue"}
            elseif ($LogEntry.Contains("send_click")) {Return "Blue"}
            else {Return "White"}
        }

        if ($LogEntry.Contains("RemoteScriptError")) {Return "Red"}
