...
class TrackRecorderPropagateNewAudioClipDecorator(TrackRecorderDecorator):
    ...
    def _propagate_new_audio_clip(self):
        ...
        duplicate_audio_clip_slots = list(
            self._get_duplicate_audio_clip_slots(source_midi_cs, source_audio_cs)
        )
        ...
        seq.add(
            [
                partial(source_audio_cs.duplicate_clip_to, clip)
                for clip in duplicate_audio_clip_slots
            ]
        )

        duplicate_midi_clips = filter(
            None,
            [self.track.midi_track.clip_slots[cs.index].clip for cs in duplicate_audio_clip_slots],
        )
        seq.add(
            [
                partial(clip.clip_name.update, source_audio_clip.clip_name.base_name)
                for clip in duplicate_midi_clips
            ]
        )

        if self.track.audio_tail_track:
            source_tail_cs = self.track.audio_tail_track.clip_slots[source_audio_cs.index]
            duplicate_audio_tail_clip_slots = [
                self.track.audio_tail_track.clip_slots[cs.index]
                for cs in duplicate_audio_clip_slots
            ]
            seq.add(
                [
                    partial(source_tail_cs.duplicate_clip_to, cs)
                    for cs in duplicate_audio_tail_clip_slots
                ]
            )

        seq.add(
            lambda: StatusBar.show_message(
                "%s audio clips duplicated" % len(duplicate_audio_clip_slots)
            )
        )
        return seq.done()
    ...
    def _get_duplicate_audio_clip_slots(self, source_midi_cs, source_audio_cs):
        ...
        for midi_cs, audio_cs in itertools.izip(
            self.track.midi_track.clip_slots, self.track.audio_track.clip_slots
        ):  # type: (MidiClipSlot, AudioClipSlot)
