...
class SceneWindow(object):
    ...
    @classmethod
    def create_from_split(cls, scene):
        # type: (Scene) -> Tuple[SceneWindow, SceneWindow]
        ...
        split_length = SongFacade.signature_numerator() * (scene.bar_length / 2)

        return cls(0, split_length, False), cls(split_length, scene.length, True)

    @classmethod
    def create_from_crop(cls, scene):
        # type: (Scene) -> SceneWindow
        crop_length = SongFacade.signature_numerator() * scene.crop_scroller.current_value
