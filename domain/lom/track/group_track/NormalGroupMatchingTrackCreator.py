...
class NormalGroupMatchingTrackCreator(MatchingTrackCreatorInterface):
    def bounce(self):
        ...
        seq.add(
            lambda: Song.selected_track().devices.mixer_device.update_from_dict(mixer_data)  # type: ignore[has-type]
        )
        seq.add(lambda: setattr(Song.selected_track().input_routing, "track", self._base_track))  # type: ignore[has-type]
