...
class AbstractActionGroup(AbstractControlSurfaceComponent):
    ...
    def add_encoder(self, identifier, name, filter_active_tracks=False, on_press=None, on_cancel_press=None, on_long_press=None,
                    on_scroll=None):
        # type: (int, str, bool, Optional[Callable], Optional[Callable], Optional[Callable], Optional[Callable]) -> MultiEncoder
        encoder = MultiEncoder(group=self, identifier=identifier, name=name, filter_active_tracks=filter_active_tracks)
        for action in EncoderAction.make_actions(name=name, on_press=on_press, on_cancel_press=on_cancel_press, on_long_press=on_long_press, on_scroll=on_scroll):
