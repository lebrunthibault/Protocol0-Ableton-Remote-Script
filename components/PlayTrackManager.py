...
class PlayTrackManager(AbstractControlSurfaceComponent):
    def action_solo_play_selected_tracks(self):
        if not self.song.has_solo_selection:
            self.song.solo_stopped_tracks = [t for t in self.song.abstract_tracks if t.is_playing and not any(t == selected_track or selected_track.is_parent(t) for selected_track in self.song.selected_tracks)]
