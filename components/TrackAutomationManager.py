...
class TrackAutomationManager(AbstractControlSurfaceComponent):
    ...
    def create_automation_group(self, base_track):
        ...
        device_name = "LFOTool_filter_automation"
        parameter_name = "F.Cutoff"
        ...
        seq.add(partial(self.parent.trackManager.create_midi_track, self.song.selected_track.index + 1,
                        name="AUTOMATION_TRACK_NAME"))
        seq.add(partial(self.parent.trackManager.create_audio_track, self.song.selected_track.index + 1,
                        name=AUTOMATION_TRACK_NAME), wait=2)
        # copying the dummy clip
        seq.add(lambda: setattr(self.song.selected_track, "has_monitor_in", True))
        seq.add(lambda: self.song.tracks[0].clip_slots[0].duplicate_clip_to(self.song.selected_track.clip_slots[0]))
