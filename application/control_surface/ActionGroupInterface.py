...
class ActionGroupInterface(object):
    ...
    def _add_multi_encoder(self, multi_encoder):
        ...
        assert (
                len([encoder for encoder in self._multi_encoders if
                     encoder.identifier == multi_encoder.identifier]) == 0
    ...
    def add_encoder(self,
        ...
        encoder = MultiEncoder(channel=self.CHANNEL - 1, identifier=identifier, name=name,
                               filter_active_tracks=filter_active_tracks, component_guard=self._component_guard)
        for action in EncoderAction.make_actions(name=name, on_press=on_press, on_long_press=on_long_press,
                                                 on_scroll=on_scroll):
