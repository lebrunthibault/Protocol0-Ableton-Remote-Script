...
class ActionGroupLog(ActionGroupMixin):
    ...
    def configure(self):
        # type: () -> None
        # LOG encoder
        self.add_encoder(identifier=1, name="log current", on_press=self._container.get(LogManager).log_current)

        # LOGS encoder
        self.add_encoder(identifier=2, name="log set", on_press=self._container.get(LogManager).log_set)
